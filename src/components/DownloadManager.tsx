...
import { 
  ...
  RefreshCw, Archive, FileText, Sliders, HardDrive, Zap, ShieldCheck 
} from 'lucide-react';
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 text-white shadow-2xl space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-base font-extrabold text-white flex items-center gap-2">
                Batch Download Manager ({completedTasks.length}/{tasks.length} Saved)
              </h3>
              <p className="text-xs text-slate-400 flex items-center gap-2">
                ...
                <span>•</span>
                <span>Concurrency: <strong className="text-purple-400">{settings.concurrencyLimit}x Parallel</strong></span>
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between text-xs font-semibold">
          <span className="text-slate-300 flex items-center gap-1.5">
            <Zap className="w-3.5 h-3.5 text-blue-400" /> Total Batch Download Progress
          </span>
      ...
      <div className="bg-slate-950/70 p-3 rounded-xl border border-slate-800 flex flex-wrap items-center justify-between gap-3 text-xs">
        <div className="flex items-center gap-2">
          ...
          <span className="text-slate-300 font-medium">Parallel Downloads Limit:</span>
        ...
        <div className="flex items-center gap-2 text-slate-400">
          ...
          <span>Preserves metadata (Author, Quality, Original Title) in file name</span>
        </div>
      </div>
        ...
        {tasks.map((task) => (
          ...
          >
            ...
            <div className="flex items-center gap-3 min-w-0 flex-1">
              <div className="relative w-12 h-12 rounded-lg bg-slate-900 overflow-hidden flex-shrink-0 border border-slate-800">
                <img
              ...
              <div className="min-w-0 space-y-1 flex-1">
                <h5 className="font-semibold text-white truncate" title={task.video.title}>
                  {task.video.title}
                </h5>
                <div className="flex items-center gap-2 text-[11px] text-slate-400">
                  ...
                  <span className="text-emerald-400 font-mono">
                    {((task.speedBps || 0) / (1024 * 1024)).toFixed(1)} MB/s
                  </span>
                </div>
            ...
            <div className="flex items-center justify-between sm:justify-end gap-3 flex-shrink-0">
              ...
              {/* Status Indicator Chip & Direct Local Save Button */}
              <div className="flex items-center gap-2">
                {task.status === 'completed' && (
                  <>
                    ...
                    {task.blobUrl && (
                      <a
                        ...
                        className="px-2.5 py-1 rounded-lg bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs flex items-center gap-1 shadow transition-all"
                        title="Re-download MP4 to Local Drive"
                      >
                ...
                {task.status === 'downloading' && (
                  <span className="px-2.5 py-1 rounded-full bg-blue-500/10 text-blue-400 border border-blue-500/30 font-semibold animate-pulse">
                    Downloading ({task.progressPercent}%)
                  </span>
                )}
                {task.status === 'queued' && (
                ...
                {task.status === 'processing_metadata' && (
                  <span className="px-2.5 py-1 rounded-full bg-purple-500/10 text-purple-400 border border-purple-500/30 font-semibold">
                    Processing Metadata
                  </span>
                )}
                {task.status === 'failed' && (
                  <span className="px-2.5 py-1 rounded-full bg-rose-500/10 text-rose-400 border border-rose-500/30 font-semibold flex items-center gap-1">
